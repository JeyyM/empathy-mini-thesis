...
# -*- coding: utf-8 -*-
"""
Fusion-Only Prediction Model for Summary Scores
Uses only fusion (combined facial+voice) features to predict summary quality
"""
...
print("="*80)
print("FUSION-ONLY PREDICTION MODEL")
print("="*80)
print("Using: Fusion (facial + voice combined) features only")
...
print(f"\n✅ Loaded {len(df)} participants with fusion emotion data")
...
print(f"Total fusion features available: {len(feature_cols)}")
...
print(f"\n📊 Top {n_features} fusion features selected:")
...
print("\n" + "="*80)
print("EVALUATING MODELS WITH FUSION FEATURES ONLY")
print("="*80)
...
pred_df.to_csv('fusion_only_predictions.csv', index=False)
print(f"\n✅ Predictions saved to: fusion_only_predictions.csv")
...
ax.set_title(f'Fusion-Only Predictions: Actual vs Predicted Summary Scores\nBest Model: {best_model["Model"]} (LOOCV) | Fusion Data | R² = {best_model["R2"]:.3f}, r = {best_model["r"]:.3f}{sig_marker}', 
...
plt.tight_layout()
plt.savefig('fusion_only_predictions.png', dpi=300, bbox_inches='tight')
print(f"✅ Visualization saved to: fusion_only_predictions.png")

print("\n" + "="*80)
print("✅ FUSION-ONLY MODEL COMPLETE!")
print("="*80)
