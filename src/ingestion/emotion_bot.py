# This came from /original_archive
...
class EmotionBot:
    ...
    def detect_emotions_fallback(self, frame):
        ...
        if len(faces) > 0:
            ...
            x, y, w, h = faces[0]
            return [{"box": (x, y, w, h), "emotions": emotions}]
    ...
    def draw_emotion_wheel(
        self, frame, arousal, valence, x_offset=20, y_offset=20, radius=100
    ):
        ...
        cv2.putText(
            frame,
            "HIGH ENERGY",
            (center_x - 45, center_y - radius - 15),
            font,
            0.5,
            (255, 200, 0),
            2,
        ...
        cv2.putText(
            frame,
            "LOW ENERGY",
            (center_x - 40, center_y + radius + 25),
            font,
            0.5,
            (100, 100, 200),
            2,
        ...
        emotion_x = int(
            center_x + max(-radius * 0.9, min(radius * 0.9, valence * radius * 0.8))
        )
        emotion_y = int(
            center_y - max(-radius * 0.9, min(radius * 0.9, arousal * radius * 0.8))
        )
        ...
        cv2.putText(
            frame,
            f"Current: {current_quadrant.upper()}",
            (x_offset, y_offset + 2 * radius + 50),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.6,
            (0, 255, 255),
            2,
        ...
        while True:
            ...
            if frame_count % frame_interval == 0:
                ...
                if emotions:
                    ...
                    quadrant = self.get_emotion_quadrant(
                        dimensions["arousal"], dimensions["valence"]
                    )
                    ...
                    print(
                        f"Frame {frame_count} ({frame_count/fps:.1f}s): {dominant_emotion} ({emotion_scores[dominant_emotion]:.3f}) | "
        ...
        while time.time() - start_time < duration_seconds:
            ...
            if current_time - last_sample >= sample_rate:
                ...
                if emotions:
                    ...
                    # Calculate enhanced dimensions
                    dimensions = self.calculate_enhanced_dimensions(emotion_scores)
                    current_quadrant = self.get_emotion_quadrant(
                        dimensions["arousal"], dimensions["valence"]
                    )
                    ...
                    cv2.rectangle(
                        frame, (x, y), (x + w, y + h), (0, 255, 0), box_thickness
                    )
                    ...
                    (conf_width, conf_height), _ = cv2.getTextSize(
                        confidence_text, font, font_scale * 0.8, thickness - 1
                    )
            ...
            # Draw emotion wheel
            frame = self.draw_emotion_wheel(
                frame, current_arousal, current_valence, frame_width - 200, 20, 80
            )
            ...
            overlay = frame.copy()
            cv2.rectangle(
                overlay, (0, 0), (frame_width - 220, overlay_height), (0, 0, 0), -1
            )
    ...
    def plot_emotions(self, save_path=None):
        ...
        for dim, color in zip(core_dims, colors):
            if dim in df.columns:
                ax2.plot(
                    x_axis, df[dim], label=dim.capitalize(), color=color, linewidth=2
                )
    ...
    def plot_heatmap(self, save_path=None, bins=20):
        ...
        ax1.text(
            0.7, -0.7, "Calm", ha="center", va="center", fontsize=10, fontweight="bold"
        )
        ax1.text(
            -0.7,
            -0.7,
        ...
        for _, row in df.iterrows():
            emotion_scores = {
                emotion: row[emotion] for emotion in emotions if emotion in row
            }
        ...
        if "elapsed_seconds" in df.columns:
            ...
            h4 = ax4.hist2d(
                time_vals, arousal_vals, bins=bins, cmap="viridis", alpha=0.8
            )
    ...
    def generate_layperson_report(self, save_path=None):
        ...
        # Main title
        fig.suptitle(
            "Your Emotion Report - Easy to Understand",
            fontsize=20,
            fontweight="bold",
            y=0.95,
        )
        ...
        emotion_totals = {
            emotion: df[emotion].sum() for emotion in emotions if emotion in df.columns
        }

        # Simplify to main emotions
        main_emotions = {
            "Positive": emotion_totals.get("happy", 0)
            + emotion_totals.get("surprise", 0),
        ...
        wedges, texts, autotexts = ax1.pie(
            main_emotions.values(),
            labels=main_emotions.keys(),
            autopct="%1.1f%%",
            colors=colors,
            startangle=90,
        )
        ...
        energy_levels = [
            (
                ...
                if x < -0.5
                    ...
                    "Low"
                    if x < -0.2
                    else "Medium" if x < 0.3 else "High" if x < 0.6 else "Very High"
                )
            )
        ...
        for i, (level, color) in enumerate(energy_colors.items()):
            ...
            if any(mask):
                ax2.scatter(
                    [j for j, m in enumerate(mask) if m],
            ...
            (
                ...
                if x < -0.5
                    ...
                    if x < -0.2
                        ...
                        "Neutral"
                        if x < 0.2
                        else "Positive" if x < 0.5 else "Very Positive"
                    )
                )
            )
        ...
        ax4.set_title(
            "Time Spent in Different Emotional States", fontsize=16, fontweight="bold"
        )
        ...
        energy_desc = (
            "high" if avg_energy > 0.2 else "low" if avg_energy < -0.2 else "moderate"
        )
        mood_desc = (
            "positive"
            if avg_mood > 0.2
            else "negative" if avg_mood < -0.2 else "neutral"
        )
...
and a generally {mood_desc} mood."""
    ...
    def plot_circle_movement_heatmap(self, save_path=None, grid_size=50):
        ...
        H, xedges, yedges = np.histogram2d(
            valence_vals, arousal_vals, bins=grid_size, range=[[-1, 1], [-1, 1]]
        )
        ...
        im1 = ax1.imshow(
            H_masked, extent=[-1, 1, -1, 1], origin="lower", cmap="YlOrRd", alpha=0.8
        )
        ...
        if len(valence_vals) > 1:
            ...
            for i in range(1, len(valence_vals)):
                dist = np.sqrt(
                    (valence_vals[i] - valence_vals[i - 1]) ** 2
            ...
            print(
                f"Movement activity: {'High' if avg_distance > 0.1 else 'Moderate' if avg_distance > 0.05 else 'Low'}"
            )
        ...
        print(f"\nExtreme positions:")
        print(
            f"  Most positive mood: {max_positive[valence_col]:.2f} ({max_positive.get(quadrant_col, 'Unknown')})"
        )
        print(
            f"  Most negative mood: {max_negative[valence_col]:.2f} ({max_negative.get(quadrant_col, 'Unknown')})"
        )
        print(
            f"  Highest energy: {max_energy[arousal_col]:.2f} ({max_energy.get(quadrant_col, 'Unknown')})"
        )
        print(
            f"  Lowest energy: {min_energy[arousal_col]:.2f} ({min_energy.get(quadrant_col, 'Unknown')})"
        )
