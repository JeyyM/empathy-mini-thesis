# Strict package-relative imports; do not touch original_archive
try:
    from .emotion_bot import EmotionBot
    from .voice_emotion_bot import VoiceEmotionBot
except Exception as e:
    raise ImportError(
        "Ingestion backends not found in src/ingestion. Ensure 'emotion_bot.py' and "
        "'voice_emotion_bot.py' exist under src/ingestion and are importable."
    ) from e
import pandas as pd
...
# MoviePy is required for audio extraction from video
try:
    try:
        import moviepy.editor as mp  # typical import path
    except Exception:
        try:
            ...
            raise ModuleNotFoundError(
                "moviepy is required for video/audio processing but editor module wasn't found."
            ) from e
except ModuleNotFoundError as e:
    raise ModuleNotFoundError(
        "moviepy is required for video/audio processing but was not found. "
        "Install dependencies with: pip install -r requirements.txt (includes moviepy)"
    ) from e
import os
...
class UnifiedEmotionTracker:
    ...
    def __init__(self, sample_rate=22050, use_ml_voice=False):
        ...
        self.voice_bot = VoiceEmotionBot(
            sample_rate=sample_rate, use_ml_model=use_ml_voice
        )
    ...
    def extract_audio_from_video(self, video_path, temp_audio_path="temp_audio.wav"):
        ...
        try:
            ...
            audio_clip.write_audiofile(
                temp_audio_path, fps=self.sample_rate, verbose=False, logger=None
            )
    ...
    def process_video_unified(self, video_path, sample_interval=1.0, cleanup_temp=True):
        ...
        if temp_audio_path and os.path.exists(temp_audio_path):
            ...
            voice_df = self.voice_bot.process_audio_file(
                temp_audio_path, segment_duration=sample_interval
            )
        ...
        synchronized_df = self.synchronize_emotion_data(
            facial_df, voice_df, sample_interval
        )
    ...
    def synchronize_emotion_data(self, facial_df, voice_df, sample_interval):
        ...
        if not facial_df.empty and not voice_df.empty:
            ...
            min_time = max(
                facial_df["time_seconds"].min(), voice_df["time_seconds"].min()
            )
            max_time = min(
                facial_df["time_seconds"].max(), voice_df["time_seconds"].max()
            )
    ...
    def plot_unified_emotions(self, save_path=None):
        ...
        axes[0].set_title(
            "🎯 Unified Emotional Dimensions (Facial + Voice)",
            fontsize=14,
            fontweight="bold",
        )
        ...
        for emotion in key_emotions:
            ...
            if voice_col in df.columns:
                axes[1].plot(
                    time_data,
                    df[voice_col],
                    label=f"Voice {emotion.title()}",
                    linewidth=2,
                    linestyle="--",
                )

        axes[1].set_title(
            "😊😠😢 Key Emotions: Facial vs Voice Comparison",
            fontsize=14,
            fontweight="bold",
        )
        ...
        if "combined_intensity" in df.columns:
            axes[3].plot(
                time_data,
                df["combined_intensity"],
                label="Combined Intensity",
                linewidth=3,
                color="black",
            )
        ...
        axes[3].set_title(
            "Combined Emotional Intensity", fontsize=14, fontweight="bold"
        )
    ...
    def get_unified_summary(self):
        ...
        if "combined_quadrant" in df.columns:
            ...
            summary["dominant_quadrant"] = (
                quadrant_counts.idxmax() if len(quadrant_counts) > 0 else "Unknown"
            )
