...
# -*- coding: utf-8 -*-
"""
Facial-Only Prediction Model for Summary Scores
Uses only facial emotion features to predict summary quality
"""
...
print("="*80)
print("FACIAL-ONLY PREDICTION MODEL")
print("="*80)
print("Using: Facial emotion features only")
print("Target: Overall Summary Score (%)")
print("Method: Leave-One-Out Cross-Validation (LOOCV)")
print("="*80)

# Load facial data
df = pd.read_csv('../correlation/facial_summary_merged.csv')
print(f"\n✅ Loaded {len(df)} participants with facial emotion data")
...
print(f"Total facial features available: {len(feature_cols)}")
...
print(f"\n📊 Top {n_features} facial features selected:")
...
print("\n" + "="*80)
print("EVALUATING MODELS WITH FACIAL FEATURES ONLY")
print("="*80)
...
pred_df.to_csv('facial_only_predictions.csv', index=False)
print(f"\n✅ Predictions saved to: facial_only_predictions.csv")
...
ax.set_title(f'Facial-Only Predictions: Actual vs Predicted Summary Scores\nBest Model: {best_model["Model"]} (LOOCV) | Facial Data | R² = {best_model["R2"]:.3f}, r = {best_model["r"]:.3f}{sig_marker}', 
...
plt.tight_layout()
plt.savefig('facial_only_predictions.png', dpi=300, bbox_inches='tight')
print(f"✅ Visualization saved to: facial_only_predictions.png")

print("\n" + "="*80)
print("✅ FACIAL-ONLY MODEL COMPLETE!")
print("="*80)
